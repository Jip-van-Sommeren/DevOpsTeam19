...
def get_item_stock_for_location(item_id, location_id):
    """
    Query the database to retrieve stock information for a specific item at a
    specific location. There is an 'item_stock' table with columns: item_id,
    location_id, and stock.
    """
    try:
...
def lambda_handler(event, context):
    ...
    Extracts item_id and location_id from the path parameters and returns
    ...
    if not item_id or not location_id:
        return {
            "statusCode": 400,
            "body": json.dumps(
                {"message": "Missing item_id or location_id in path"}
            ),
        }
